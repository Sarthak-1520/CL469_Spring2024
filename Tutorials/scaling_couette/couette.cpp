    ...
    for(int i=0;i<LX;i++)
    {
        for(int j=0; j<LY;j++)
        {
            ...
            {//apply collision for non-solid nodes only
                ...
                for (int k=0; k<beta ; k++)
                {
                    f_temp[i][j][k] = f[i][j][k]*(1.0- 1.0/tau) +
                        (1.0/tau)*f_eq[k] ;
                }
            }
        }
    ...
    for(int i=1;i<LX-1;i++)
    {
        for(int j=1; j<LY-1; j++)
        {
            for (int k=0; k<beta ; k++)
            {
                ...
                if(obstacle[destinationX][destinationY] == 0)
                {
                    f[i][j][k] = f_temp[destinationX][destinationY][k];
                }
                else
                {
                    f[i][j][k] = f_temp[i][j][INVLIST[k]] -
                        6.0 * WLIST[INVLIST[k]] * rho[i][j] *
                        (cX[INVLIST[k]] * u_x[destinationX][destinationY] +
                         cY[INVLIST[k]] * u_y[destinationX][destinationY]);
                }
            }
        }
    }
...
{
    ofstream veloFile;
    veloFile.open("output/velocity_field_" + to_string(time) + ".dat");
    for(int i = 0; i < LX; i++)
    {
        for(int j = 0; j < LY; j++)
        {
            veloFile<<i<<"\t"<<j<<"\t"<<u_x[i][j]<<"\t"<<u_y[i][j]<<"\n";
        }
    }
    veloFile.close();
    ofstream densityFile;
    densityFile.open("output/density_field_" + to_string(time) + ".dat");
    for(int i = 0; i < LX; i++)
    {
        for(int j = 0; j < LY; j++)
        {
            densityFile<<i<<"\t"<<j<<"\t"<<rho[i][j]<<"\t"
                <<3.0 * rho[i][j]<<"\t"<<obstacle[i][j]<<"\n";
        }
    }
    densityFile.close();
}
